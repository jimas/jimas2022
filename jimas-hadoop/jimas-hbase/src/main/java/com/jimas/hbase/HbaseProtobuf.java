...
public class HbaseProtobuf {
    ...
    public void createTable() throws IOException {
        TableName tableName = TableName.valueOf("phone_buf");
    ...
    public void testScan() throws Exception {
        Scan scan = new Scan();
        //start：减大值，stop：减小值
        scan.withStartRow(Bytes.toBytes("17699691383_" + (Long.MAX_VALUE - sdf.parse("20220228000000").getTime())));
        scan.withStopRow(Bytes.toBytes("17699691383_" + (Long.MAX_VALUE - sdf.parse("20220212000000").getTime())));
        ResultScanner scanner = table.getScanner(scan);
        for (Result result : scanner) {
            System.out.print(Bytes.toString(CellUtil.cloneRow(result.getColumnLatestCell(Bytes.toBytes("cf"), Bytes.toBytes("phone")))));
            byte[] bytes = CellUtil.cloneValue(result.getColumnLatestCell(Bytes.toBytes("cf"), Bytes.toBytes("phone")));
            Phone.Request request = Phone.Request.parseFrom(bytes);
            System.out.print("--" + request.getDest());
            System.out.print("--" + request.getType());
            System.out.print("--" + request.getLength());
            System.out.println(request.getCallTime());
        }
    }
    ...
    public void insertDemoData() throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            for (int j = 0; j < 1000; j++) {
                ...
                String type = String.valueOf(RandomUtils.nextInt(0, 1));
                String callTime = getTime("2022");
                Put put = new Put(Bytes.toBytes(tellA + "_" + (Long.MAX_VALUE - sdf.parse(callTime).getTime())));
                Phone.Request build = Phone.Request.newBuilder()
                        .setDest(dest)
                        .setCallTime(callTime)
                        .setLength(length)
                        .setType(type).build();
                put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("phone"), build.toByteArray());
                list.add(put);
            }
        }
        table.put(list);
